  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold">Hóa đơn của tôi</h2>
          <p className="text-muted-foreground mt-1">
            {isAdminOrWorker 
              ? "Quản lý và theo dõi công nợ khách hàng." 
              : "Theo dõi công nợ và tình trạng thanh toán của bạn."}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 glass-card rounded-2xl border-l-4 border-l-destructive">
          <p className="text-sm text-muted-foreground font-medium mb-1">
            {isAdminOrWorker ? "Tổng nợ chưa thu" : "Tổng nợ của bạn"}
          </p>
        ...
        <div className="p-6 glass-card rounded-2xl border-l-4 border-l-emerald-500">
          <p className="text-sm text-muted-foreground font-medium mb-1">
            {isAdminOrWorker ? "Đã thu trong tháng" : "Đã thanh toán (Tháng này)"}
          </p>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            type="text"
            placeholder={isAdminOrWorker ? "Tìm mã hóa đơn hoặc tên khách hàng..." : "Tìm mã hóa đơn..."}
        ...
        <div className="flex gap-2">
          <button className="px-4 py-2 bg-card border border-border rounded-xl hover:bg-accent transition-all flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-accent/50 text-muted-foreground text-[10px] uppercase tracking-wider font-bold">
                <th className="p-6">Hóa đơn</th>
                {isAdminOrWorker && <th className="p-6">Khách hàng</th>}
                <th className="p-6 text-right">Tiêu thụ</th>
                <th className="p-6 text-right">Tổng tiền</th>
                <th className="p-6 text-center">Trạng thái</th>
                <th className="p-6">Hạn thanh toán</th>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                <tr>
                  <td colSpan={isAdminOrWorker ? 7 : 6} className="p-12 text-center text-muted-foreground">Đang tải hóa đơn...</td>
                </tr>
              ) : bills?.length === 0 ? (
                <tr>
                  <td colSpan={isAdminOrWorker ? 7 : 6} className="p-12 text-center text-muted-foreground">Không có hóa đơn nào.</td>
                </tr>
              ) : (
                bills?.map((bill, i) => (
                  <motion.tr 
                  ...
                  >
                    ...
                    <td className="p-6 text-right font-medium">{bill.consumption} m³</td>
                    <td className="p-6 text-right font-bold text-primary">
